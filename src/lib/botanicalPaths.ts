/**
 * Fonte única dos traços botânicos do site.
 * BackgroundPattern (folha da página) e SurfacePattern (dentro dos blocos)
 * devem importar daqui — nunca reinventar paths por componente.
 */

export type BotanicalVariant = "branch" | "leaf-cluster" | "vine";

export type BotanicalPathRole = "stem" | "stem-olive" | "stem-quiet" | "leaf" | "leaf-olive" | "vein";

export type BotanicalPath = {
  d: string;
  role: BotanicalPathRole;
};

export const BOTANICAL_VIEWBOX: Record<BotanicalVariant, string> = {
  branch: "0 0 1000 760",
  "leaf-cluster": "0 0 1000 760",
  vine: "0 0 1000 1300",
};

export const BOTANICAL_PATHS: Record<BotanicalVariant, readonly BotanicalPath[]> = {
  branch: [
    {
      role: "stem-olive",
...
export function botanicalPathClassName(role: BotanicalPathRole): string {
  switch (role) {
    case "stem":
      return "background-pattern__stem";
    case "stem-olive":
      return "background-pattern__stem background-pattern__stem--olive";
    case "stem-quiet":
      return "background-pattern__stem background-pattern__stem--quiet";
    case "leaf":
      return "background-pattern__leaf";
    case "leaf-olive":
      return "background-pattern__leaf background-pattern__leaf--olive";
    case "vein":
      return "background-pattern__vein";
  }
}
