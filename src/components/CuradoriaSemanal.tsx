...
function CuradoriaVideo({ src, poster, label }: { src: string; poster: string; label: string }) {
  ...
  return (
    <video ref={videoRef} autoPlay loop muted playsInline preload="metadata" poster={poster} aria-label={`Video do prato ${label}`} className="h-full w-full object-cover">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {PRATOS_DA_SEMANA.map((prato, index) => (
            <FadeIn key={prato.id} delay={0.08 + index * 0.06}>
              <article className="group relative aspect-[9/16] overflow-hidden rounded-2xl border border-hairline bg-surface">
                <CuradoriaVideo src={cloudinaryVideoUrl(prato.cloudinaryPublicId)} poster={cloudinaryVideoPoster(prato.cloudinaryPublicId)} label={prato.nome} />
