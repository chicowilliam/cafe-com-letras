...
export function ReservationFab() {
  ...
  return (
    <button
      type="button"
      onClick={open}
      className={`btn-primary focus-ring fixed z-50 flex !cursor-pointer items-center gap-2 rounded-full px-4 py-3 text-sm font-medium shadow-lg shadow-black/40 transition-[transform,opacity] duration-300 ease-out active:scale-[0.98] motion-reduce:transition-none bottom-[max(1.25rem,env(safe-area-inset-bottom))] left-[max(1.25rem,env(safe-area-inset-left))] md:gap-2.5 md:px-5 md:py-3.5 ${
        visible
