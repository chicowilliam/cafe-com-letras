...
export function ExperienceCard({
  ...
  imageAlt,
  badge,
  badgeClassName = "border-accent/30 bg-black/65 text-accent",
  title,
  subtitle,
  description,
  highlights,
  ...
  return (
    <article className="card-experience group flex h-full flex-col overflow-hidden md:flex-row">
      <div className="relative h-48 w-full shrink-0 overflow-hidden md:h-auto md:w-1/3 md:min-h-[168px]">
        ...
        />
        <span
          className={`absolute left-2.5 top-2.5 flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium tracking-tight backdrop-blur-md ${badgeClassName}`}
        >
          {badge}
        </span>
      </div>
