...
if (-not (Test-Path $backup)) { throw "Missing .commit-backup — run full script from clean state first." }

function Write-Utf8([string]$Path, [string]$Content) {
  [System.IO.File]::WriteAllText((Resolve-Path -LiteralPath (Split-Path $Path -Parent) -ErrorAction SilentlyContinue | ForEach-Object { Join-Path $Path } ), $Content)
}
...
# 3
Write-Utf8File "src/components/Cardapio.tsx" @'
...
Write-Utf8File "src/lib/navigation.ts" @'
...
Write-Utf8File "src/styles/route-transitions.css" @'
...
$css = Get-Content src/styles/route-transitions.css -Raw
$css += @'
...
'@
Write-Utf8File "src/styles/route-transitions.css" $css
...
$css = Get-Content src/styles/route-transitions.css -Raw
$css = $css -replace '(?s)@media \(prefers-reduced-motion: reduce\) \{.*\}\s*$', ''
$css += @'
...
'@
Write-Utf8File "src/styles/route-transitions.css" $css.TrimEnd()
...
Write-Utf8File "src/App.tsx" $app.TrimEnd()
...
Write-Host "Done: commits 3-21 created."
git log -21 --oneline
